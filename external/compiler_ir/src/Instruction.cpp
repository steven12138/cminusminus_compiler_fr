...
void CmpInst::assertValid()
{
    ...
    assert(Type::is_eq_type(lhs_ty, rhs_ty) && "cmp operands must match");
    assert((lhs_ty->is_integer_type() || lhs_ty->is_float_type()) &&
           "cmp only supports int/float");
}
...
std::string CmpInst::print()
{
    ...
    instr_ir += this->get_name();
    instr_ir += " = ";
    bool is_float = this->get_operand(0)->get_type()->is_float_type();
    instr_ir += is_float ? "fcmp" : this->get_module()->get_instr_op_name(this->get_instr_type());
    instr_ir += " ";
    instr_ir += print_cmp_type(this->cmp_op_, is_float);
    instr_ir += " ";
    instr_ir += this->get_operand(0)->get_type()->print();
    instr_ir += " ";
    instr_ir += print_as_op(this->get_operand(0), false);
...
ZextInst *ZextInst::create_sitofp(Value *val, Type *ty, BasicBlock *bb)
{
    return new ZextInst(Instruction::sitofp, val, ty, bb);
}

ZextInst *ZextInst::create_fptosi(Value *val, Type *ty, BasicBlock *bb)
{
    return new ZextInst(Instruction::fptosi, val, ty, bb);
}
