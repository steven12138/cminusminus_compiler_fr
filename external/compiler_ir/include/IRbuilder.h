...
public:
  ...
  */
  ...
  /*!
   *@brief 创建 int 到 float 的转换指令
   */
  ZextInst *create_sitofp(Value *val, Type *ty) {
    return ZextInst::create_sitofp(val, ty, this->BB_);
  }
  /*!
   *@brief 创建 float 到 int 的转换指令
   */
  ZextInst *create_fptosi(Value *val, Type *ty) {
    return ZextInst::create_fptosi(val, ty, this->BB_);
  }
};
